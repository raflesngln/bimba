...
$dari=($hal*$max)-$max;
  ...
  if($status=='semua' && $level=='semua'){
   $cari=mysql_query("select * from siswa limit $dari,$max");
  }
  else if($status=='semua' and $level !='semua'){
	$cari=mysql_query("select * from siswa where id_level='$level' limit $dari,$max");
  }
    else if($status !='semua' and $level =='semua'){
	$cari=mysql_query("select * from siswa where status='$status' limit $dari,$max");  
  }
   else if($status !='semua' and $level !='semua'){
	$cari=mysql_query("select * from siswa where status='$status'  AND id_level='$level' limit $dari,$max");  
  }
